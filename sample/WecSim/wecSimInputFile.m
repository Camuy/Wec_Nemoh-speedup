...
simu.rampTime = 3;                            	% Wave Ramp Time [s]
...
body(2).mass = 17.1497;        % volume*rho            
body(2).inertia = [ 17.1497*7^2  17.1497*7^2 0.3455];
body(2).centerGravity = [0 0 -7];
body(2).volume = 0.0156;
body(2).initial.displacement = [0 0 1.4];
...
constraint(1).hardStops.upperLimitBound = 0.2;
constraint(1).hardStops.upperLimitDamping = 100;
constraint(1).hardStops.upperLimitStiffness = 100;
constraint(1).hardStops.lowerLimitSpecify = 'off';
constraint(1).hardStops.lowerLimitBound = -0.2;
constraint(1).hardStops.lowerLimitDamping = 100;
constraint(1).hardStops.lowerLimitStiffness = 100;

% Translational PTO
pto(1) = ptoClass('PTO1');                      % Initialize PTO Class for PTO1
pto(1).stiffness = 0;                           % PTO Stiffness [N/m]
pto(1).damping = 200;                           % PTO Damping [N/(m/s)]
pto(1).location = [0 0 0];                      % PTO Location [m]
pto(1).equilibriumPosition = 0;                 % PTO Equilibrium Position [m]
pto(1).hardStops.upperLimitSpecify = 'on';
pto(1).hardStops.upperLimitBound = 0.1;
pto(1).hardStops.upperLimitDamping = 0;
pto(1).hardStops.upperLimitStiffness = 500;
pto(1).hardStops.lowerLimitSpecify = 'on';
pto(1).hardStops.lowerLimitBound = -0.1;
pto(1).hardStops.lowerLimitDamping = 0;
pto(1).hardStops.lowerLimitStiffness = 500;
